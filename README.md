# Olá, eu sou o Paulo Fernandes! 👋

Atuo na área de Operações e Telecomunicações e estou em transição de carreira para a tecnologia, atualmente cursando **Análise e Desenvolvimento de Sistemas** pelo Senac. Busco unir minha sólida experiência em liderança operacional e processos técnicos com o desenvolvimento de softwares eficientes.

---

### 🛠️ Tecnologias e Ferramentas
* **Linguagens e Fundamentos:** Lógica de Programação, Python, HTML5, CSS3, JavaScript
* **Ferramentas Técnicas:** AutoCAD

---

### 💼 Experiência em Destaque
* **Operador de Vendas e Serviços de Campo** – Brisanet Telecomunicações (Atual)
* **Supervisor / Inspetor de Operações** – Proxxima Telecom (Gestão de equipes, indicadores e qualidade)
* **Instalador e Reparador de Redes** – Proxxima Telecom (Infraestrutura e fibra óptica)

---

### 🎓 Formação e Certificações
* **Graduação:** Tecnólogo em Análise e Desenvolvimento de Sistemas – Senac (Em andamento)
* **Curso Complementar:** DevStart (Lógica, HTML, CSS, JS) – SENAI
* **Gestão:** Gestão de Pessoas – Escola do Sebrae

---

### 📈 Minhas Metas Acadêmicas
* Criar projetos práticos aplicando lógica estruturada e POO em Python.
* Desenvolver interfaces Web responsivas e integradas.
* Construir um portfólio sólido de ferramentas focadas em automação e melhoria contínua de processos.

---

### 📫 Como me encontrar
* **LinkedIn:** [://linkedin.com](https://www.://linkedin.com)
* **E-mail:** paulofernandesoficial7@gmail.com
